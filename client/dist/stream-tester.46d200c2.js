 ...
 */ class P5Visualizer {
   ...
   */ displaySkeletonFrame(frame) {
        if (!this.p5Instance) {
            console.error('Cannot display skeleton: p5 instance not available');
            return;
        }
        // Clear canvas
        this.p5Instance.background(255);
   ...
   */ _drawSkeleton(body, color) {
        ...
        joints.forEach((joint, index)=>{
            ...
            if ('depthX' in joint && 'depthY' in joint) {
                ...
                const canvasWidth = p51.width;
                const canvasHeight = p51.height;
                ...
                // Draw joint
                p51.ellipse(x, y, 10, 10);
                if (window.DEBUG && window.DEBUG.DATA && index === 0) console.log(`Using camera coordinates for joint ${index}: x=${x}, y=${y}`);
            } else if (window.DEBUG && window.DEBUG.DATA && index === 0) console.warn(`Joint ${index} missing coordinate data:`, joint);
        });
        ...
        connections.forEach(([i, j])=>{
            if (joints[i] && joints[j]) {
                // Check if we have depthX/depthY coordinates
                if ('depthX' in joints[i] && 'depthY' in joints[i] && 'depthX' in joints[j] && 'depthY' in joints[j]) p51.line(joints[i].depthX * p51.width, joints[i].depthY * p51.height, joints[j].depthX * p51.width, joints[j].depthY * p51.height);
                else if ('cameraX' in joints[i] && 'cameraY' in joints[i] && 'cameraX' in joints[j] && 'cameraY' in joints[j]) {
                    // Fallback to camera coordinates
                    const scaleFactor = 0.5; // Adjust this value as needed
                    const centerX = p51.width / 2;
...
//# sourceMappingURL=stream-tester.46d200c2.js.map
