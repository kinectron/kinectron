 ...
 */ class ThreeVisualizer {
   ...
   */ initialize() {
        ...
        this.camera = new THREE.PerspectiveCamera(45, this.renderer.domElement.width / this.renderer.domElement.height, 1, 10000);
   ...
   */ _createParticles() {
        ...
        for(let y = 0; y < this.DEPTHHEIGHT; y++)for(let x = 0; x < this.DEPTHWIDTH; x++){
            ...
            this.colors[i] = new THREE.Color(0.5 + normalizedX * 0.5, 0.5 + normalizedY * 0.5, 0.5);
        }
   ...
   */ updatePointCloud(depthValues, filterZeros = false) {
        ...
        if (Math.random() < 0.01 && window.DEBUG.RAW_DEPTH && window.DEBUG.DATA) {
            // Only log occasionally
            // Sample a few points to check their z values
            const samplePoints = [];
            for(let i = 0; i < 5; i++){
                const idx = Math.floor(Math.random() * this.particles.vertices.length);
                samplePoints.push({
                    index: idx,
...
//# sourceMappingURL=stream-tester.063731ce.js.map
