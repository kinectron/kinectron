...
function setupConnectionEvents() {
    ...
    kinectron.on('error', (error)=>{
        statusDiv.html('Connection status: <strong>Error</strong> - ' + error.message);
    });
    // When connection state changes
    kinectron.on('stateChange', (data)=>{
        statusDiv.html('Connection status: <strong>' + data.to + '</strong>');
    });
}
// --- KEY CONTROLS ---
function keyPressed() {
    if (keyCode === ENTER) startColorStream();
    else if (keyCode === UP_ARROW) startDepthStream();
    else if (keyCode === RIGHT_ARROW) stopAllStreams();
}
// --- STREAM CONTROLS ---
// Start the color stream
function startColorStream() {
    statusDiv.html('Connection status: <strong>Connected</strong> - Color stream active');
    activeStream = 'color';
    kinectron.startColor(drawFeed);
}
// Start the depth stream
function startDepthStream() {
    statusDiv.html('Connection status: <strong>Connected</strong> - Depth stream active');
    activeStream = 'depth';
    kinectron.startDepth(drawFeed);
}
// Stop all streams
function stopAllStreams() {
    statusDiv.html('Connection status: <strong>Connected</strong> - No active streams');
    activeStream = null;
    kinectron.stopAll();
    background(0);
}
...
function initializeKinect() {
    statusDiv.html('Connection status: <strong>Connected</strong> - Initializing Kinect...');
    kinectron.initKinect().then((result)=>{
        if (result.success || result.alreadyInitialized) statusDiv.html('Connection status: <strong>Connected</strong> - Kinect initialized');
        else statusDiv.html('Connection status: <strong>Error</strong> - Failed to initialize Kinect');
    }).catch((error)=>{
        statusDiv.html('Connection status: <strong>Error</strong> - ' + error.message);
    });
}
...
function drawFeed(frame) {
    ...
    loadImage(frame.src, function(loadedImage) {
        ...
        if (loadedImage.width !== canvasWidth || loadedImage.height !== canvasHeight) {
...
//# sourceMappingURL=gettingstarted.b18ee69c.js.map
